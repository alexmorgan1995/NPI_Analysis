...
plot(beta1(seq(0,730), 71, (6*7), 0.5))
...
SIRS <- function(time, state, parameters) {
  with(as.list(c(state, parameters)), {
    dSv = - beta1(time,tstart1,tdur,scaling) * (
         (1 + delta1*(beta1(time,tstart1,tdur,scaling) + beta1(time,tstart1,tdur,scaling)) * 
                                           ((f_v*f_v)/ (f_v*f_v + f_r*f_r)))) * Iv*Sv - 
      beta1(time,tstart1,tdur,scaling)*Ih*Sv -
      beta4(time,tstart1)*Ir*Sv + zeta*Rv
    ...
    dSr = - beta3(time,tstart1,tdur,scaling) * (
         (1 + delta1*(beta1(time,tstart1,tdur,scaling) + beta1(time,tstart1,tdur,scaling)) *
                                          ((f_r*f_r)/ (f_v*f_v + f_r*f_r)))) * Ir*Sr - 
      beta2(time,tstart1,tdur,scaling)*Ih*Sr -
      beta4(time,tstart1)*Iv*Sr + zeta*Rr
    
    
    dIv = beta1(time,tstart1,tdur,scaling) * (
      (1+delta1*(beta1(time,tstart1,tdur,scaling) + beta1(time,tstart1,tdur,scaling)) * 
                                           ((f_v*f_v)/ (f_v*f_v + f_r*f_r)))) * Iv*Sv  + 
      beta1(time,tstart1,tdur,scaling)*Ih*Sv +
      beta4(time,tstart1)*Ir*Sv - gamma*Iv
    
    dIh = beta2(time,tstart1,tdur,scaling)*Ih*Sh + 
      beta2(time,tstart1,tdur,scaling)*Ir*Sh +
      beta1(time,tstart1,tdur,scaling)*Iv*Sh - gamma*Ih
    
    dIr = beta3(time,tstart1,tdur,scaling) * (
      (1+delta1*(beta1(time,tstart1,tdur,scaling) + beta1(time,tstart1,tdur,scaling)) *
                                          ((f_r*f_r)/ (f_v*f_v + f_r*f_r)))) * Ir*Sr +
      beta2(time,tstart1,tdur,scaling)*Ih*Sr +
      beta4(time,tstart1)*Iv*Sr - gamma*Ir
...
parms = c(gamma = 1/(GenTime(3.3,2.8)), 
          ...
          f_r = 0.6,
          f_v = 0.2,
          f_h = 0.2) #CAN VARY THIS - DEPENDING ON FACTOR EXPLORED

gamma = 1/(GenTime(3.3,2.8)); zeta = 1/365; tstart1 = 71; tdur = 6*7; scaling = 0.5; delta1 = 0.5; f_r = 0.6; f_v = 0.2; f_h = 0.2

out1 <- data.frame(ode(y = init, func = SIRS, times = time, parms = parms))
...
out1$Betavv <- beta1(time,tstart1,tdur,scaling) * (1 + delta1*(beta1(time,tstart1,tdur,scaling) + beta1(time,tstart1,tdur,scaling)) * 
     ((f_v*f_v)/ (f_v*f_v + f_r*f_r))) #VARY THIS ASWELL
out1$Betavh_hv <- beta1(seq(0,730), 71, (6*7), 0.5)
out1$Betahh_hr_rh <- beta2(seq(0,730), 71, (6*7), 0.5)
out1$Betarr <- beta3(time,tstart1,tdur,scaling) * (1+delta1*(beta1(time,tstart1,tdur,scaling) + beta1(time,tstart1,tdur,scaling)) * ((f_r*f_r)/ (f_v*f_v + f_r*f_r))) 
out1$Betarv_vr <- beta4(seq(0,730), 71)

colnames(out1) <- c("Time", "Suscv", "Susch", "Suscnv", "Infected_Iv", "Infected_Ih", "Infected_Ir", 
                    "Recovv", "Recovh", "Recovnv", "Betavv", "Beta1vh_hv", "Betahh_hr_rh", "Betarr", "Betarv_vr")

statsinfecv <- melt(out1, id.vars = c("Time"), measure.vars = c("Infected_Iv", "Infected_Ih", "Infected_Ir"))
statsbeta1 <- melt(out1, id.vars =  c("Time"), measure.vars = c("Betavv"))
statsbeta2 <- melt(out1, id.vars =  c("Time"), measure.vars = c("Beta1vh_hv"))
statsbeta3 <- melt(out1, id.vars =  c("Time"), measure.vars = c("Betahh_hr_rh"))
statsbeta4 <- melt(out1, id.vars =  c("Time"), measure.vars = c("Betarr"))
statsbeta5 <- melt(out1, id.vars =  c("Time"), measure.vars = c("Betarv_vr"))
...
pbeta2 <- ggplot(data = statsbeta2, aes(x = (Time), y = value, col = variable)) + geom_line(size = 1.02, stat = "identity") +
  labs(x ="Time (Days)", y = "Beta_vh/hv") + scale_y_continuous(limits = c(0,0.5),  expand = c(0,0)) +
  theme(legend.position = "none", legend.title = element_text(size=14), legend.text=element_text(size=14),  axis.text=element_text(size=14),
        axis.title.y=element_text(size=14),axis.title.x= element_blank(), 
        legend.spacing.x = unit(0.3, 'cm'), plot.margin=unit(c(0.7,0.7,0.8,0.8),"cm")) + scale_x_continuous(expand = c(0, 0)) 
