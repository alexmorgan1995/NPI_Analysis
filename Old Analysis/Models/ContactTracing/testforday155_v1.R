...
Input <- c(10880, 459384.8)

Input <- c(850)


init <- c(S = (N-((Input[[1]]*(1/parms[["gamma"]])) + (0.075*N)))/N, I = Input[[1]]*(1/parms[["gamma"]])/N, R = 0.075)
#eta(parms["R0"])*init["I"]*init["S"]*parms["N"] 
inc_cont <<- data.frame(0, 850, 0, 0, 0)
...
N <- 5.5*10^6
Input <- c(10880, 459384.8)
init <- c(S = (N-(Input[1] + Input[2]))/N, I = Input[1]/N, R = Input[2]/N)
