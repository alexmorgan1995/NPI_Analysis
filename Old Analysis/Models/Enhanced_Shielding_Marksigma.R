...
SIRS <- function(time, state, parameters) {
  with(as.list(c(state, parameters)), {
    dSv = - beta1(time,tstart1,tdur,scaling)*Iv*Sv*sigma  - 
      beta1(time,tstart1,tdur,scaling)*Ih*Sv*sigma -
      beta4(time,tstart1)*delta*Ir*Sv*sigma  + zeta*Rv
    
    dSh = - beta2(time,tstart1,tdur,scaling)*Ih*Sh*sigma - 
      beta2(time,tstart1,tdur,scaling)*Ir*Sh*sigma -
      beta1(time,tstart1,tdur,scaling)*Iv*Sh*sigma + zeta*Rh
    
    dSr = - beta3(time,tstart1,tdur,scaling)*Ir*Sr*sigma - 
      beta2(time,tstart1,tdur,scaling)*Ih*Sr*sigma -
      beta4(time,tstart1)*delta*Iv*Sr*sigma + zeta*Rr
    
    
    dIv = beta1(time,tstart1,tdur,scaling)*Iv*Sv*sigma + 
      beta1(time,tstart1,tdur,scaling)*Ih*Sv*sigma +
      beta4(time,tstart1)*delta*Ir*Sv*sigma - gamma*Iv
    
    dIh = beta2(time,tstart1,tdur,scaling)*Ih*Sh*sigma + 
      beta2(time,tstart1,tdur,scaling)*Ir*Sh*sigma +
      beta1(time,tstart1,tdur,scaling)*Iv*Sh*sigma - gamma*Ih
    
    dIr = beta3(time,tstart1,tdur,scaling)*Ir*Sr*sigma  + 
      beta2(time,tstart1,tdur,scaling)*Ih*Sr*sigma +
      beta4(time,tstart1)*delta*Iv*Sr - gamma*Ir*sigma
...
parms = c(gamma = 1/(GenTime(3.3,2.8)), 
          ...
          bv = 0.6/0.2,
          bn = 0.2/0.6,
          sigma = 1.086) #CAN VARY THIS - DEPENDING ON FACTOR EXPLORED

out1 <- data.frame(ode(y = init, func = SIRS, times = times, parms = parms))

out1$Beta_vv <- beta1(seq(0,730),71,(6*7), 0.5)* as.numeric(parms[9])
out1$Beta_vh_hv <- beta1(seq(0,730), 71, (6*7), 0.5)* as.numeric(parms[9])
out1$Beta_hh_rh_hr <- beta2(seq(0,730), 71, (6*7), 0.5)* as.numeric(parms[9])
out1$Beta_rr <- beta3(seq(0,730),71,(6*7), 0.5)* as.numeric(parms[9])
out1$Beta_rv_vr <- beta4(seq(0,730), 71)*as.numeric(parms[6])* as.numeric(parms[9])
...
out1$recovtot <- out1$Recovv + out1$Recovh + out1$Recovnv
out1$inftot <- out1$Infected_Iv + out1$Infected_Ih + out1$Infected_Ir

statsinfectotal <- melt(out1, id.vars = c("Time"), measure.vars = c("inftot"))
statsrecov <- melt(out1, id.vars = c("Time"), measure.vars = c("recovtot"))

#### Aggregated Plots ####
pinf<- ggplot(data = statsinfecv, aes(x = (Time), y = value, col = variable)) + geom_line(size = 1.02, stat = "identity") +
...
pinftot <- ggplot(data = statsinfectotal, aes(x = (Time), y = value)) + geom_line(size = 1.02, stat = "identity", col = "darkred") +
  labs(x ="Time (Days)", y = "Proportion Total Infected") + scale_y_continuous(limits = c(0,0.05),  expand = c(0,0)) +
  theme(legend.position = "bottom", legend.title = element_text(size=14), legend.text=element_text(size=14),  axis.text=element_text(size=14),
        axis.title.y=element_text(size=14),axis.title.x= element_blank(), 
        legend.spacing.x = unit(0.3, 'cm'), plot.margin=unit(c(0.7,0.7,0.8,0.8),"cm")) + scale_x_continuous(expand = c(0, 0))

precov <- ggplot(data = statsrecov, aes(x = (Time), y = value)) + geom_line(size = 1.02, stat = "identity", col = "darkgreen") +
  labs(x ="Time (Days)", y = "Proportion Total Infected") + scale_y_continuous(limits = c(0,1),  expand = c(0,0)) +
  theme(legend.position = "bottom", legend.title = element_text(size=14), legend.text=element_text(size=14),  axis.text=element_text(size=14),
        axis.title.y=element_text(size=14),axis.title.x= element_blank(), 
        legend.spacing.x = unit(0.3, 'cm'), plot.margin=unit(c(0.7,0.7,0.8,0.8),"cm")) + scale_x_continuous(expand = c(0, 0))


ggarrange(pinf, pinftot, precov, nrow = 3, ncol = 1, heights = c(0.2, 0.2, 0.2)) 
