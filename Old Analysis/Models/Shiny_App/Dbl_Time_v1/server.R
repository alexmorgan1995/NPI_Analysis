...
# 2) COMPUTE DOUBLING TIME  ----
  ...
  # GET USER INPUT DATES
    ...
    output$progressBox <- renderValueBox({
      ...
      valueBox(tags$p(paste0("Doubling time: ", Td, " Days"), style = "font-size: 50%;"),
  ...
  output$progressBox1 <- renderValueBox({
    ...
    valueBox(
      ...
      paste0("Between ", as.character(d2$date[1]), " and ", as.character(d2$date[nrow(d2)])),
      color = "green"
    )
  })
  ...
  output$instructions <- renderUI({
    HTML(paste("<strong>This application calculates the doubling time and the resulting epidemic curve for 14 of Scotland's NHS healthboards.</strong>",
               ...
               "",
               "The start date for the doubling time calculations must be made where the number of cases is <strong>AT LEAST 1</strong> for the specified healthboard.",
               "",
