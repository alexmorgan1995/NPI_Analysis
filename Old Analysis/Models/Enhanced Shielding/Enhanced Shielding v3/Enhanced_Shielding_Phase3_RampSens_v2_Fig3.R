setwd("C:/Users/amorg/Documents/PhD/nCoV Work/Figures/Enhanced Shielding/New") # This is where the Plots will be Output
rm(list=ls())
...
beta1 <- function(time, tstart1, tdur, tdur2) {
  ...
  ifelse((time >= tstart1 & time <= tstart1+tdur), #Phase 2
         0.8*(gamma),
         ifelse((time >= tstart1+tdur & time <= tstart1+tdur+(tdur2*7)), #Phase 3
                betalin(time),
...
parms = c(gamma = 1/(GenTime(3.3,2.8)), 
          ...
          tstart1 = 71, 
          tdur = 6*7,
          tdur2 = 12) #CAN VARY THIS - DEPENDING ON FACTOR EXPLORED
...
phase3 <- data.frame(xmin=71+(6*7), xmax=71+(6*7)+(as.numeric(parms[5])*7), ymin=-Inf, ymax=Inf, name = "P3")
phase4 <- data.frame(xmin=71+(6*7)+(as.numeric(parms[5])*7), xmax=Inf, ymin=-Inf, ymax=Inf, name = "P4")
...
parms = c(gamma = 1/(GenTime(3.3,2.8)), 
          ...
          tstart1 = 71, 
          tdur = 6*7,
          tdur2 = 6) #CAN VARY THIS - DEPENDING ON FACTOR EXPLORED
...
parms = c(gamma = 1/(GenTime(3.3,2.8)), 
          ...
          tstart1 = 71, 
          tdur = 6*7,
          tdur2 = 18) #CAN VARY THIS - DEPENDING ON FACTOR EXPLORED
...
ggsave(p1, filename = "Fig3v2.png", dpi = 300, type = "cairo",
