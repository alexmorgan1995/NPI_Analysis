...
#### Trigger Day Sensitivity ####
#Explore the different Trigger Days Manually 
#### Baseline Parameters - Day 71 ####
...
#Scaling to a total pop size of 1
...
#Adding a column detailing the Betas
...
pinf71 <- ggplot(data = statsinfecv, aes(x = (Time), y = value, col = variable))  + theme_bw() +
  ...
  geom_text(data = phase1, aes(x = xmin, y = 0.1, label = name),inherit.aes = FALSE, size = 8, vjust = 0, hjust = 0, nudge_x = 25) +
  geom_text(data = phase2, aes(x = xmin, y = 0.1, label = name),inherit.aes = FALSE, size = 8, vjust = 0, hjust = 0, nudge_x = 8) +
  geom_text(data = phase3, aes(x = xmin, y = 0.1, label = name),inherit.aes = FALSE, size = 8, vjust = 0, hjust = 0, nudge_x = 30) +
  geom_text(data = phase4, aes(x = xmin, y = 0.1, label = name),inherit.aes = FALSE, size = 8, vjust = 0, hjust = 0, nudge_x = 125) +
...
pinf46 <- ggplot(data = statsinfecv, aes(x = (Time), y = value, col = variable))  + theme_bw() +
  ...
  geom_rect(data=phase3, aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax), fill="grey", alpha=0.35, inherit.aes = FALSE) +
  geom_rect(data=phase4, aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax), fill="grey", alpha=0.5, inherit.aes = FALSE) +
  geom_text(data = phase1, aes(x = xmin, y = 0.1, label = name),inherit.aes = FALSE, size = 8, vjust = 0, hjust = 0, nudge_x = 11) +
  geom_text(data = phase2, aes(x = xmin, y = 0.1, label = name),inherit.aes = FALSE, size = 8, vjust = 0, hjust = 0, nudge_x = 8) +
  geom_text(data = phase3, aes(x = xmin, y = 0.1, label = name),inherit.aes = FALSE, size = 8, vjust = 0, hjust = 0, nudge_x = 25) +
  geom_text(data = phase4, aes(x = xmin, y = 0.1, label = name),inherit.aes = FALSE, size = 8, vjust = 0, hjust = 0, nudge_x = 125) +
  geom_line(size = 1.02, stat = "identity")
...
out1$Beta1 <- beta1(times, 96, (6*7), as.numeric(parms[5])) 
out1$Beta2 <- beta2(times, 96, (6*7), as.numeric(parms[5])) 
out1$Beta3 <- beta3(times, 96, (6*7), as.numeric(parms[5]))
out1$Beta4 <- beta4(times, 96, (6*7), as.numeric(parms[5]))
...
#Adding the intervention phases
...
pinf96 <- ggplot(data = statsinfecv, aes(x = (Time), y = value, col = variable))  + theme_bw() +
  ...
  geom_rect(data=phase3, aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax), fill="grey", alpha=0.35, inherit.aes = FALSE) +
  geom_rect(data=phase4, aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax), fill="grey", alpha=0.5, inherit.aes = FALSE) +
  geom_text(data = phase1, aes(x = xmin, y = 0.1, label = name),inherit.aes = FALSE, size = 8, vjust = 0, hjust = 0, nudge_x = 36) +
  geom_text(data = phase2, aes(x = xmin, y = 0.1, label = name),inherit.aes = FALSE, size = 8, vjust = 0, hjust = 0, nudge_x = 7) +
  geom_text(data = phase3, aes(x = xmin, y = 0.1, label = name),inherit.aes = FALSE, size = 8, vjust = 0, hjust = 0, nudge_x = 27) +
  geom_text(data = phase4, aes(x = xmin, y = 0.1, label = name),inherit.aes = FALSE, size = 8, vjust = 0, hjust = 0, nudge_x = 110) +
  geom_line(size = 1.02, stat = "identity")
...
beta1 <- function(time, tstart1, tdur, scaling) {
  gamma <- 1/(GenTime(3.3,2.8))
  beta1_2 <- (0.6*(gamma))*scaling
  betalin <- approxfun(x=c(tstart1+tdur, tstart1+tdur+(12*7)), y = c(0.6*(gamma), beta1_2), method="linear", rule  =2)
  ifelse((time >= tstart1 & time <= tstart1+tdur), #Phase 2
         0.6*(gamma),
         ifelse((time >= tstart1+tdur & time <= tstart1+tdur+(12*7)), #Phase 3
                betalin(time),
...
beta2 <- function(time, tstart1, tdur, scaling) {
  gamma <- 1/(GenTime(3.3,2.8))
  beta1_2 <- (2.8*(gamma) - ((2.8*(gamma) - 0.7*(gamma))*scaling))
  betalin <- approxfun(x=c(tstart1+tdur, tstart1+tdur+(12*7)), y = c(0.7*(gamma), beta1_2), method="linear", rule  =2)
  ifelse((time >= tstart1 & time <= tstart1+tdur), #Phase 2
         0.7*(gamma),
         ifelse((time >= tstart1+tdur & time <= tstart1+tdur+(12*7)), #Phase 3
                betalin(time),
...
beta1 <- function(time, tstart1, tdur, scaling) {
  gamma <- 1/(GenTime(3.3,2.8))
  beta1_2 <- (1*(gamma))*scaling
  betalin <- approxfun(x=c(tstart1+tdur, tstart1+tdur+(12*7)), y = c(1*(gamma), beta1_2), method="linear", rule  =2)
  ifelse((time >= tstart1 & time <= tstart1+tdur), #Phase 2
         1*(gamma),
         ifelse((time >= tstart1+tdur & time <= tstart1+tdur+(12*7)), #Phase 3
                betalin(time),
...
beta2 <- function(time, tstart1, tdur, scaling) {
  gamma <- 1/(GenTime(3.3,2.8))
  beta1_2 <- (2.8*(gamma) - ((2.8*(gamma) - 1.1*(gamma))*scaling))
  betalin <- approxfun(x=c(tstart1+tdur, tstart1+tdur+(12*7)), y = c(1.1*(gamma), beta1_2), method="linear", rule  =2)
  ifelse((time >= tstart1 & time <= tstart1+tdur), #Phase 2
         1.1*(gamma),
         ifelse((time >= tstart1+tdur & time <= tstart1+tdur+(12*7)), #Phase 3
                betalin(time),
...
ggplot(data = statsbeta1, aes(x = (Time), y = value, lty = variable)) + theme_bw() +
  ...
  geom_rect(data=phase3, aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax), fill="grey", alpha=0.35, inherit.aes = FALSE) +
  geom_rect(data=phase4, aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax), fill="grey", alpha=0.5, inherit.aes = FALSE) +
  geom_text(data = phase1, aes(x = xmin, y = 0.303, label = name),inherit.aes = FALSE, size = 8, vjust = 0, hjust = 0, nudge_x = 35) +
  geom_text(data = phase2, aes(x = xmin, y = 0.303, label = name),inherit.aes = FALSE, size = 8, vjust = 0, hjust = 0, nudge_x = 15) +
  geom_text(data = phase3, aes(x = xmin, y = 0.303, label = name),inherit.aes = FALSE, size = 8, vjust = 0, hjust = 0, nudge_x = 35) +
  geom_text(data = phase4, aes(x = xmin, y = 0.303, label = name),inherit.aes = FALSE, size = 8, vjust = 0, hjust = 0, nudge_x = 125) +
  geom_line(size = 1.02, stat = "identity", col = "black")

#### Plotting ####

#ggarrange(pinf71, NULL, pinf46, NULL, pinf96, nrow = 5, ncol = 1, align = "v", heights = c(0.5, -0.03, 0.5, -0.03, 0.6), labels = c("A"), font.label = c(size = 20) )
#ggarrange(pinf0809, NULL, pinf0607, NULL, pinf1011, nrow = 5, ncol = 1, align = "v", heights = c(0.5, -0.03, 0.5, -0.03, 0.6), labels = c("B"), font.label = c(size = 20) )
...
ggsave(plot, filename = "Fig4_v1.png", dpi = 300, type = "cairo",
