...
input <- data.frame("Time" = 41, "R0" = 1.5, "doublingtime" = 4.6, "ReducBeta" = 0.4, "LenInt" = 12)
...
plot(seq(0,365), betastatdecrease(seq(0,365, by = 1), 
                 input$Time, 
                 input$Time+(input$LenInt*7),
                 input$R0*(1/(GenTime(input$doublingtime,input$R0))),
                 input$R0*(1/(GenTime(input$doublingtime,input$R0)))*(1-input$ReducBeta)), ylim = c(0, 1))
...
input <- data.frame("Time" = 41, "R0" = 2.4, "doublingtime" = 4.6, "ReducBeta" = 0.4)
...
title = "Population by the End of the Outbreak (Cumulative Infections)"
