...
plot(beta1(seq(0,730), 100, (6*7), (24*7), 0.097))
...
SIRnoss <- function(time, state, parameters) {
  with(as.list(c(state, parameters)), {
    dSv = - beta2(time,tstart1,tdur1,tdur2)*Iv*Sv - beta1(time,tstart1,tdur1,tdur2,beta1_2)*Inv*Sv 
    
    dSnv = - beta2(time,tstart1,tdur1,tdur2)*Inv*Snv - beta2(time,tstart1,tdur1,tdur2)*Iv*Snv 
    
    dIv = beta2(time,tstart1,tdur1,tdur2)*Iv*Sv + beta1(time,tstart1,tdur1,tdur2,beta1_2)*Inv*Sv - gamma*Iv
    
    dInv =  beta2(time,tstart1,tdur1,tdur2)*Inv*Snv + beta2(time,tstart1,tdur1,tdur2)*Iv*Snv - gamma*Inv
    
    dR = gamma*Iv + gamma*Inv
    
    dCv = beta2(time,tstart1,tdur1,tdur2)*Iv*Sv + beta1(time,tstart1,tdur1,tdur2,beta1_2)*Inv*Sv
...
out1 <- data.frame(ode(y = init, func = SIRnoss, times = times, parms = parms))
...
statsrecovnoss <- melt(out1, id.vars = c("Time"), measure.vars = c("Recov"))
...
out2$beta1 <- beta1(seq(0,730), 100, (6*7), (24*7), 0)
...
statsinfecss$model <- "Effect on BetaSS"; statsinfecnoss$model <- "No Effect on BetaSS"
statsinfeccomb <- rbind.data.frame(statsinfecss, statsinfecnoss)

statsrecovss$model <- "Effect on BetaSS"; statsrecovnoss$model <- "No Effect on BetaSS"
statsrecovcomb <- rbind.data.frame(statsrecovss, statsrecovnoss)

statsbetass$model <- "Effect on BetaSS"; statsbetanoss$model <- "No Effect on BetaSS"
...
pinfnv <- ggplot(data = statsinfeccomb[statsinfeccomb$variable == "Infected_Ip",], aes(x = (Time), y = value, col = model)) + geom_line(size = 1.02, stat = "identity") +
  labs(x ="Time (Days)", y = "Proportion of Vulnerable Infected") + scale_y_continuous(limits = c(0,0.035) ,  expand = c(0,0)) +
  theme(legend.position = "none", legend.title = element_blank(), legend.text=element_text(size=14),  axis.text=element_text(size=14), axis.title.y=element_text(size=11),axis.title.x= element_blank(),
        legend.spacing.x = unit(0.3, 'cm'), plot.margin=unit(c(0.7,0.7,0.8,0.8),"cm")) +
  scale_x_continuous(expand = c(0, 0)) + scale_color_manual(values = c("darkred", "darkblue")) 

prec <- ggplot(data = statsrecovcomb, aes(x = (Time), y = value, col = model)) + geom_line(size = 1.02, stat = "identity") +
