...
#Plotting 
ggplot(data = stats1[which(stats1$variable == "Infec"),], aes(x = (Time), y = (value)*100, col = Lockdown_Efficacy)) + geom_line(size = 1.02, stat = "identity") +
  labs(x ="Time Relative to Lockdown Date (Days)", y = "Percentage of Population Infected (%)") + scale_y_continuous(limits = c(0,7.5) ,  expand = c(0,0)) + 
  ...
  scale_x_continuous(expand = c(0, 0), limits = c(-50, 75)) + labs(color='Level of Compliance:') +
  scale_color_manual(values=c("black", "#1F78B4", "#33A02C",  "darkgreen","darkred"))  +
  annotate("rect", xmin = 0, 
           xmax = 21, ymin = 0, ymax = 7.5, fill = "darkred", alpha = .3) +
  geom_segment(aes(x=-50, xend=62, y=1.82, yend=1.82), color = "black", size = 1.02, lty = 2)
