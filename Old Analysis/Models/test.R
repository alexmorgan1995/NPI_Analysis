...
beta41 <- function(time, tstart1) {
  ifelse((time >= tstart1 & time <= 730), #Phase 2
         0, 
         (1.7/2)*(1/(GenTime(3.3,2.8)))
  )
}
  ...
  with(as.list(c(state, parameters)), {
    ...
    if(time < tstart1) { #Same but introduce Ci
      dSv = - beta1(time,tstart1,tdur,scaling)*Iv*Sv - beta1(time,tstart1,tdur,scaling)*Ih*Sv -
        beta1(time,tstart1,tdur,scaling)*cb*In*Sv + zeta*Rv
      ...
      dSn = - beta3(time,tstart1,tdur,scaling)*In*Sn - beta2(time,tstart1,tdur,scaling)*Ih*Sn -
        beta1(time,tstart1,tdur,scaling)*cb*Iv*Sn  + zeta*Rn
      
      dIv = beta1(time,tstart1,tdur,scaling)*Iv*Sv + beta1(time,tstart1,tdur,scaling)*Ih*Sv +
        beta1(time,tstart1,tdur,scaling)*cb*In*Sv - gamma*Iv
      ...
      dIn = beta3(time,tstart1,tdur,scaling)*In*Sn + beta2(time,tstart1,tdur,scaling)*Ih*Sn +
        beta1(time,tstart1,tdur,scaling)*cb*Iv*Sn - gamma*In
      ...
      dSv = - beta1(time,tstart1,tdur,scaling)*Iv*Sv - beta1(time,tstart1,tdur,scaling)*Ih*Sv + zeta*Rv
      
      dSh = - beta2(time,tstart1,tdur,scaling)*Ih*Sh - beta2(time,tstart1,tdur,scaling)*In*Sh -
        beta1(time,tstart1,tdur,scaling)*Iv*Sh + zeta*Rh
      
      dSn = - beta3(time,tstart1,tdur,scaling)*In*Sn - beta2(time,tstart1,tdur,scaling)*Ih*Sn + zeta*Rn
      
      dIv = beta1(time,tstart1,tdur,scaling)*Iv*Sv + beta1(time,tstart1,tdur,scaling)*Ih*Sv - gamma*Iv
      
      dIh = beta2(time,tstart1,tdur,scaling)*Ih*Sh + beta2(time,tstart1,tdur,scaling)*In*Sh +
        beta1(time,tstart1,tdur,scaling)*Iv*Sh - gamma*Ih
      
      dIn = beta3(time,tstart1,tdur,scaling)*In*Sn + beta2(time,tstart1,tdur,scaling)*Ih*Sn  - gamma*In
...
out1$Beta4 <- beta41(seq(0,730), 71)

colnames(out1) <- c("Time", "Suscv", "Susch", "Suscnv", "Infected_Iv", "Infected_Ih", "Infected_Ir", 
                    "Recovv", "Recovh", "Recovnv", "Beta1", "Beta2", "Beta3", "Beta4")
...
statsbeta1 <- melt(out1, id.vars =  c("Time"), measure.vars = c("Beta1"))
statsbeta2 <- melt(out1, id.vars =  c("Time"), measure.vars = c("Beta2"))
statsbeta3 <- melt(out1, id.vars =  c("Time"), measure.vars = c("Beta3"))
statsbeta4 <- melt(out1, id.vars =  c("Time"), measure.vars = c("Beta4"))

#### Aggregated Plots ####

pinfv <- ggplot(data = statsinfecv, aes(x = (Time), y = value, col = variable)) + geom_line(size = 1.02, stat = "identity") +
...
ggarrange(pinfv, pbeta1, pbeta2, pbeta3, pbeta4, nrow = 5, ncol = 1, heights = c(0.4, 0.2, 0.2, 0.2, 0.2)) 
