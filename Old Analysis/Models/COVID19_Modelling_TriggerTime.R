...
#Decrease in Beta - Scenario 1
basebeta <- 2*(1/(GenTime(6,2)))
alteredbeta <- 2*(1/(GenTime(6,2)))*(1-0.375)

#Decrease in Beta - All Other Scenarios 
basebeta <- 2*(1/(GenTime(6,2)))
alteredbeta <- (2*(1/(GenTime(6,2)))*(1-0.375))/2
...
betadecrease <- function(time, int_timestart, int_timeend, betastart, betaend) {
  betalin <- approxfun(x=c(int_timestart, int_timeend+1),y= c(betastart, betaend), method="linear", rule  =2)
  ifelse((time >= int_timestart & time <= int_timeend+1),
         betalin(time),
         (2*(1/(GenTime(6,2)))))
}

beta <- data.frame(seq(0,365))
beta$time <- betadecrease(times, 43, 43+(12*7), 
                          (2*(1/(GenTime(6,2)))*(1-0.375))/2, (2*(1/(GenTime(6,2)))))
...
trigday <- seq(1,100, by = 1)
stats2 <- data.frame(matrix(nrow = length(trigday), ncol = 4)); colnames(stats2) <- c("TrigDay", "TimePeak", "PeakInf", "CumInf")

for(i in 1:length(trigday)) {
  parms = c(mu = 1/(GenTime(6,2)), 
            int_timestart = trigday[i], 
            int_timeend = trigday[i]+(12*7), 
            beta_base = (2*(1/(GenTime(6,2)))*(1-0.375))/2,
            beta_int = 2*(1/(GenTime(6,2))))
  out2 <- data.frame(ode(y = init, func = SIR2, times = times, parms = parms))
  stats2[i,1] <- parms[2]
  stats2[i,2] <- out2[,1][which(out2[,3] == max(out2[,3]))] #Time
  stats2[i,3] <- out2[,3][which(out2[,3] == max(out2[,3]))] #No Inf
  stats2[i,4] <- max(out2[,5]) 
  print(i/length(trigday))
}

stats2[which.min(stats2$PeakInf),]

parms = c(mu = 1/(GenTime(6,2)),
          int_timestart = as.numeric(stats2[which.min(stats2$PeakInf),][1]), 
          int_timeend = as.numeric(stats2[which.min(stats2$PeakInf),][1])+(12*7),
          beta_base = (2*(1/(GenTime(6,2)))*(1-0.375))/2,
          beta_int = 2*(1/(GenTime(6,2))))
...
betadecrease <- function(time, int_timestart, int_timeend, betastart, betaend) {
  betalin <- approxfun(x=c(int_timestart-1, int_timeend),y= c(betastart, betaend), method="linear", rule  =2)
  ifelse((time >= int_timestart-1 & time <= int_timeend),
         betalin(time),
         (2*(1/(GenTime(6,2)))))
}
...
beta <- data.frame(seq(0,365))
beta$time <- betadecrease(times, 43, 43+(12*7), 
                          (2*(1/(GenTime(6,2)))),(2*(1/(GenTime(6,2)))*(1-0.375))/2)
...
trigday <- seq(1,100, by = 1)
stats3 <- data.frame(matrix(nrow = length(trigday), ncol = 4)); colnames(stats3) <- c("TrigDay", "TimePeak", "PeakInf", "CumInf")

for(i in 1:length(trigday)) {
  parms = c(mu = 1/(GenTime(6,2)), int_timestart = trigday[i], int_timeend = trigday[i]+(12*7), beta_base = 2*(1/(GenTime(6,2))),
            beta_int = (2*(1/(GenTime(6,2)))*(1-0.375))/2)
  out3 <- data.frame(ode(y = init, func = SIR3, times = times, parms = parms))
  stats3[i,1] <- parms[2]
  stats3[i,2] <- out3[,1][which(out3[,3] == max(out3[,3]))] #Time
  stats3[i,3] <- out3[,3][which(out3[,3] == max(out3[,3]))] #No Inf
  stats3[i,4] <- max(out3[,5]) 
  print(i/length(trigday))
}

stats3[which.min(stats3$PeakInf),]

parms = c(mu = 1/(GenTime(6,2)),
          int_timestart = as.numeric(stats3[which.min(stats3$PeakInf),][1]), 
          int_timeend = as.numeric(stats3[which.min(stats3$PeakInf),][1])+(12*7),
...
betaincdec <- function(time, int_timestart, int_timeend, betastart, betaend) {
  betadec <- approxfun(x=c(int_timestart-1, (int_timestart-1)+abs(((int_timestart-1)-(int_timeend+1))/2)), y= c(betastart, betaend), 
                       method="linear", rule = 2)
  betainc <- approxfun(x=c((int_timestart-1)+abs(((int_timestart-1)-(int_timeend+1))/2), int_timeend+1),y= c(betaend, betastart), 
                       method="linear", rule = 2)
  ifelse((time >= int_timestart & time <= int_timeend),
...
beta <- data.frame(seq(0,365))
beta$time <- betaincdec(times, 43, 43+(12*7), 
                   (2*(1/(GenTime(6,2)))),(2*(1/(GenTime(6,2)))*(1-0.375))/2)
...
trigday <- seq(1,100, by = 1)
stats4 <- data.frame(matrix(nrow = length(trigday), ncol = 4)); colnames(stats4) <- c("TrigDay", "TimePeak", "PeakInf", "CumInf")
...
for(i in 1:length(trigday)) {
  ...
  stats4[i,1] <- parms[2]
  stats4[i,2] <- out4[,1][which(out4[,3] == max(out4[,3]))] #Time
  stats4[i,3] <- out4[,3][which(out4[,3] == max(out4[,3]))] #No Inf
  stats4[i,4] <- max(out3[,5]) 
  print(i/length(trigday))
}

stats4[which.min(stats4$PeakInf),]

parms = c(mu = 1/(GenTime(6,2)),
          int_timestart = as.numeric(stats4[which.min(stats4$PeakInf),][1]), 
          int_timeend = as.numeric(stats4[which.min(stats4$PeakInf),][1])+(12*7),
          beta_base = (2*(1/(GenTime(6,2)))),
          beta_int = (2*(1/(GenTime(6,2)))*(1-0.375))/2)
