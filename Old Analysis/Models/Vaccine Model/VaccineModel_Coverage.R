...
out$S_i[out$time == 271] +  out$S_j[out$time == 271] +  out$S_k[out$time == 271]
out$I_i[out$time == 271] +  out$I_j[out$time == 271] +  out$I_k[out$time == 271]
out$R_i[out$time == 271] +  out$R_j[out$time == 271] +  out$R_k[out$time == 271]
...
datatext <- data.frame(x = c(182.5, 182.5, 182.5, 182.5), y = c(0.245, 0.235, 0.225, 0.215), 
...
if(parms[["simp"]] == "No") {
  ggsave(pcomb, filename = paste0("vacc_scenario_Coverage_", parms[["P_i"]],".png"), dpi = 300, type = "cairo", width = 10, height = 14, units = "in")
} else{
  ggsave(pcomb, filename = paste0("vacc_scenario_Transblock_",parms[["P_i"]] ,"_simple_", parms[["timing"]] ,".png"), dpi = 300, type = "cairo", width = 10, height = 14, units = "in")
}

if(parms[["simp"]] == "No") {
  ggsave(p_noint_i, filename = paste0("total_vacc_scenario_Transblock_", parms[["P_i"]],"_seq.png"), dpi = 300, type = "cairo", width = 10, height = 10, units = "in")
}
