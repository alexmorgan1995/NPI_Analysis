...
if(parms[["simp"]] == "No") {
  data_i$group <- "prerel14_seq"
} else{
  data_i$group <- paste0("prerel14_simple_", parms[["timing"]])
  paste0("prerel_14_simple_", parms[["timing"]])
}

if(parms[["simp"]] == "No") {
  write.csv(data_i,'vacc_scenario_prerel14_seq.csv')
} else{
  write.csv(data_i,paste0("prerel_14_simple_", parms[["timing"]] ,".csv"))
}
