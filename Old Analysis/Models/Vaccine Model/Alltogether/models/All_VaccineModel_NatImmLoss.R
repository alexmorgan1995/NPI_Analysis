...
if(parms[["simp"]] == "No") {
  data_i$group <- "NatImmLoss_seq"
} else{
  data_i$group <- paste0("NatImmLoss_simple_", parms[["timing"]])
  paste0("NatImmLoss_simple_", parms[["timing"]])
}

if(parms[["simp"]] == "No") {
  write.csv(data_i,'vacc_scenario_NatImmLoss_seq.csv')
} else{
  write.csv(data_i,paste0("NatImmLoss_simple_", parms[["timing"]] ,".csv"))
}
