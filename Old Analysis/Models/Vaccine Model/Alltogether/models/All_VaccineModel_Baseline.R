...
data_i <- melt(out, id.vars = c("time"), measure.vars = c("I_i"), value.name = "I")


if(parms[["simp"]] == "No") {
  data_i$group <- "simple_baseline"
} else{
  data_i$group <- paste0("simple_", parms[["timing"]])
  paste0("simple_", parms[["timing"]])
}

if(parms[["simp"]] == "No") {
  write.csv(data_i,'vacc_scenario_baseseq.csv')
} else{
  write.csv(data_i,paste0("vacc_scenario_simple_", parms[["timing"]] ,".csv"))
}
