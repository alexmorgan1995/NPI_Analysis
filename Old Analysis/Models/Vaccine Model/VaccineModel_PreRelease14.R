...
betafunc <- function(time, T_i, dt_i, T_j, dt_j, T_k, dt_k, socdist) {
  beta <- matrix(data = (1.4*gamma), nrow = 3, ncol = 3, dimnames = list(c("i", "j", "k"), c("i", "j", "k"))) 
  
  if(time > (T_i + dt_i)) {
    beta[1,] <- beta[1,]*3
    beta[2:3,1] <- beta[2:3,1]*3
  }
  
  if(time > (T_j + dt_j)) {
    beta[2,2:3] <- beta[2,2:3]*3
    beta[3,2] <- beta[3,2]*3
  }
...
if(parms[["simp"]] == "No") {
  ggsave(pcomb, filename = paste0("vacc_scenario_prerel14_seq.png"), dpi = 300, type = "cairo", width = 10, height = 14, units = "in")
} else{
  ggsave(pcomb, filename = paste0("vacc_scenario_prerel_14_simple_", parms[["timing"]] ,".png"), dpi = 300, type = "cairo", width = 10, height = 14, units = "in")
}

if(parms[["simp"]] == "No") {
  ggsave(p_noint_i, filename = "total_vacc_scenario_prerel14_seq.png", dpi = 300, type = "cairo", width = 10, height = 10, units = "in")
}
