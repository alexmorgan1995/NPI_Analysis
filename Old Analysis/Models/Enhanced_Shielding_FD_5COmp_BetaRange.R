...
r0data <- rbind.data.frame(data.frame("Phase" = c("Phase 0"), "Times" = seq(0,70),                                  "OverallR0" = c(1.7), "VulR0" = c(1.6999), "ShiR0" = c(1.6999), "RemR0" = c(1.6999)),
                           ...
                           data.frame("Phase" = c("Phase 2"), "Times" = seq(71+(6*7),71+(6*7)+(12*7)-1),            "OverallR0" = c(NA), "VulR0" = c(NA), "ShiR0" = c(NA), "RemR0" = c(NA)),
                           data.frame("Phase" = c("Phase 3"), "Times" = seq(71+(6*7)+(12*7), 71+(6*7)+(365)),       "OverallR0" = c(1.63), "VulR0" = c(0.4), "ShiR0" = c(1.27), "RemR0" = c(1.8)))
...
pbeta <- ggplot(data = statsbeta1, aes(x = (Time), y = value, lty = variable)) + theme_bw() +
  ...
  geom_rect(data=phase3, aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax), fill="grey", alpha=0.35, inherit.aes = FALSE) +
  geom_rect(data=phase4, aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax), fill="grey", alpha=0.5, inherit.aes = FALSE) +
  geom_text(data = phase1, aes(x = xmin, y = 0.303, label = name),inherit.aes = FALSE, size = 8, vjust = 0, hjust = 0, nudge_x = 25) +
  geom_text(data = phase2, aes(x = xmin, y = 0.303, label = name),inherit.aes = FALSE, size = 8, vjust = 0, hjust = 0, nudge_x = 8) +
  geom_text(data = phase3, aes(x = xmin, y = 0.303, label = name),inherit.aes = FALSE, size = 8, vjust = 0, hjust = 0, nudge_x = 30) +
...
ggarrange(pinf, NULL, pbeta,NULL,pr0, nrow = 5, ncol =1, align = "v", heights = c(0.5, -0.03, 0.5,-0.03, 0.5), labels = c("A","", "B","","C"), font.label = c(size = 20) )

#### Beta Range ####
#Beta1
t1down1 <- approxfun(x=c(0, (12*7)), y = c(0.0700,0.0350), method="linear", rule  =2)
t1down2 <- approxfun(x=c(0, (12*7)), y = c(0.1167,0.0583), method="linear", rule  =2)

beta11 <- data.frame(times = seq(0,478),
                    beta1 = beta1(times, 71, (6*7), 0.5),
                    lowerbound = c(rep(0.1634,71), rep(0.0700, 43), t1down1(seq(1,84)), rep(0.0350, 281)),
                    higherbound = c(rep(0.2333,71), rep(0.1167, 43), t1down2(seq(1,84)), rep(0.0583, 281)))

beta111 <- ggplot(data = beta11, aes(x = times, y = beta1)) + theme_bw()  +
  labs(x ="Time (Days)", y = "??1 & ??4", lty = " ") + scale_y_continuous(limits = c(0,0.3),  expand = c(0,0)) +
  theme(legend.position = "bottom", legend.title = element_text(size=14), legend.text=element_text(size=14),  axis.text=element_text(size=14),
        axis.title.y=element_text(size=14),axis.title.x= element_blank(), 
        legend.spacing.x = unit(0.3, 'cm'), plot.margin=unit(c(0.7,0.7,0.8,0.8),"cm")) + scale_x_continuous(expand = c(0, 0)) + 
  geom_ribbon(aes(ymin = lowerbound, ymax = higherbound), fill = "grey70") + geom_line(size = 1.02, col = "black")
  
#Beta2
t2up1 <- approxfun(x=c(0, (12*7)), y = c(0.0817,0.2042), method="linear", rule  =2)
t2up2 <- approxfun(x=c(0, (12*7)), y = c(0.128,0.2275), method="linear", rule  =2)

beta12 <- data.frame(times = seq(0,478),
                     beta1 = beta2(times, 71, (6*7), 0.5),
                     lowerbound = c(rep(0.1634,71), rep(0.0817, 43), t2up1(seq(1,84)), rep(0.2042, 281)),
                     higherbound = c(rep(0.2333,71), rep(0.128, 43), t2up2(seq(1,84)), rep(0.2275, 281)))

beta211 <- ggplot(data = beta12, aes(x = times, y = beta1)) + theme_bw()  +
  labs(x ="Time (Days)", y = "??2", lty = " ") + scale_y_continuous(limits = c(0,0.3),  expand = c(0,0)) +
  theme(legend.position = "bottom", legend.title = element_text(size=14), legend.text=element_text(size=14),  axis.text=element_text(size=14),
        axis.title.y=element_text(size=14),axis.title.x= element_blank(), 
        legend.spacing.x = unit(0.3, 'cm'), plot.margin=unit(c(0.7,0.7,0.8,0.8),"cm")) + scale_x_continuous(expand = c(0, 0)) + 
  geom_ribbon(aes(ymin = lowerbound, ymax = higherbound), fill = "grey70") + geom_line(size = 1.02, col = "black")
...
#Beta3
t3up1 <- approxfun(x=c(0, (12*7)), y = c(0.0817,0.2626), method="linear", rule  =2)
t3up2 <- approxfun(x=c(0, (12*7)), y = c(0.128,0.2626), method="linear", rule  =2)

beta13 <- data.frame(times = seq(0,478),
                     beta1 = beta3(times, 71, (6*7), 0.5),
                     lowerbound = c(rep(0.1634,71), rep(0.0817, 43), t3up1(seq(1,84)), rep(0.2626, 281)),
                     higherbound = c(rep(0.2333,71), rep(0.128, 43), t3up2(seq(1,84)), rep(0.2626, 281)))

beta311 <- ggplot(data = beta13, aes(x = times, y = beta1)) + theme_bw()  +
  labs(x ="Time (Days)", y = "??3", lty = " ") + scale_y_continuous(limits = c(0,0.3),  expand = c(0,0)) +
  theme(legend.position = "bottom", legend.title = element_text(size=14), legend.text=element_text(size=14),  axis.text=element_text(size=14),
        axis.title.y=element_text(size=14),axis.title.x= element_text(size=14), 
        legend.spacing.x = unit(0.3, 'cm'), plot.margin=unit(c(0.7,0.7,0.8,0.8),"cm")) + scale_x_continuous(expand = c(0, 0)) + 
  geom_ribbon(aes(ymin = lowerbound, ymax = higherbound), fill = "grey70") + geom_line(size = 1.02, col = "black")

ggarrange(beta111, NULL, beta211,NULL,beta311, nrow = 5, ncol =1, align = "v", heights = c(0.5, -0.03, 0.5,-0.03, 0.5), labels = c("A","", "B","","C"), font.label = c(size = 20) )