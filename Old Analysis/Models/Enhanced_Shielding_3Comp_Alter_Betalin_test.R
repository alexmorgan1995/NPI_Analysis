...
SIRS <- function(time, state, parameters) {
  with(as.list(c(state, parameters)), {
      dSv = - beta1(time,tstart1,tdur,scaling)*Iv*Sv - 
        ...
        beta4(time,tstart1)*In*Sv - 
        beta1(time,tstart1,tdur,scaling)*bv*0.5*(1-cb)*Iv*Sv - 
        beta1(time,tstart1,tdur,scaling)*bv*0.5*(1-cb)*Ih*Sv  + zeta*Rv
      
      dSh = - beta2(time,tstart1,tdur,scaling)*Ih*Sh - 
        beta2(time,tstart1,tdur,scaling)*In*Sh -
        beta1(time,tstart1,tdur,scaling)*Iv*Sh + zeta*Rh
      
      dSn = - beta3(time,tstart1,tdur,scaling)*In*Sn - 
        beta2(time,tstart1,tdur,scaling)*Ih*Sn -
        beta4(time,tstart1)*Iv*Sn - 
        beta3(time,tstart1,tdur,scaling)*bn*(1-cb)*In*Sn + zeta*Rn
      ...
      dIn = beta3(time,tstart1,tdur,scaling)*In*Sn + 
        beta2(time,tstart1,tdur,scaling)*Ih*Sn +
        beta4(time,tstart1)*Iv*Sn + 
        beta3(time,tstart1,tdur,scaling)*bn*(1-cb)*In*Sn - gamma*In
