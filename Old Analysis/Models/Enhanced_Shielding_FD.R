...
beta2 <- function(time, tstart1, tdur, scaling1) {
  beta1_2 <- (2.8*(1/(GenTime(3.3,2.8))) - ((2.8*(1/(GenTime(3.3,2.8))) - 0.9*(1/(GenTime(3.3,2.8))))*scaling1))
...
beta3 <- function(time, tstart1, tdur, scaling1) {
  beta1_2 <- (2.8*(1/(GenTime(3.3,2.8))) - ((2.8*(1/(GenTime(3.3,2.8))) - 1.7*(1/(GenTime(3.3,2.8))))*scaling1))
  betalin <- approxfun(x=c(tstart1+tdur, tstart1+tdur+(12*7)), y = c(0.9*(1/(GenTime(3.3,2.8))), beta1_2), method="linear", rule  =2)
  ifelse((time >= tstart1 & time <= tstart1+tdur), #Phase 2
         0.9*(1/(GenTime(3.3,2.8))),
         ifelse((time >= tstart1+tdur & time <= tstart1+tdur+(12*7)), #Phase 3
...
SIRS <- function(time, state, parameters) {
  with(as.list(c(state, parameters)), {
    dSv = - beta1(time,tstart1,tdur,scaling)*(1/3)*(Iv/f_v)*Sv - 
      beta1(time,tstart1,tdur,scaling)*(1/3)*(Ih/f_h)*Sv -
      beta4(time,tstart1,tdur,scaling)*(1/3)*(Ir/f_r)*Sv + zeta*Rv # delta multiplies the equation but found in the function
    
    dSh = - beta2(time,tstart1,tdur,scaling1)*(1/3)*(Ih/f_h)*Sh - 
      beta2(time,tstart1,tdur,scaling1)*(1/3)*(Ir/f_r)*Sh -
      beta1(time,tstart1,tdur,scaling)*(1/3)*(Iv/f_v)*Sh + zeta*Rh
    
    dSr = - beta3(time,tstart1,tdur,scaling1)*(1/3)*(Ir/f_r)*Sr - 
      beta2(time,tstart1,tdur,scaling1)*(1/3)*(Ih/f_h)*Sr -
      beta4(time,tstart1,tdur,scaling)*(1/3)*(Iv/f_v)*Sr + zeta*Rr # delta multiplies the equation but found in the function
    
    dIv = beta1(time,tstart1,tdur,scaling)*(1/3)*(Iv/f_v)*Sv + 
      beta1(time,tstart1,tdur,scaling)*(1/3)*(Ih/f_h)*Sv +
      beta4(time,tstart1,tdur,scaling)*(1/3)*(Ir/f_r)*Sv - gamma*Iv # delta multiplies the equation but found in the function
    
    dIh = beta2(time,tstart1,tdur,scaling1)*(1/3)*(Ih/f_h)*Sh + 
      beta2(time,tstart1,tdur,scaling1)*(1/3)*(Ir/f_r)*Sh +
      beta1(time,tstart1,tdur,scaling)*(1/3)*(Iv/f_v)*Sh - gamma*Ih
    
    dIr = beta3(time,tstart1,tdur,scaling1)*(1/3)*(Ir/f_r)*Sr  + 
      beta2(time,tstart1,tdur,scaling1)*(1/3)*(Ih/f_h)*Sr +
      beta4(time,tstart1,tdur,scaling)*(1/3)*(Iv/f_v)*Sr - gamma*Ir # delta multiplies the equation but found in the function
...
parms = c(gamma = 1/(GenTime(3.3,2.8)), 
          ...
          f_v = 0.2,
          f_h = 0.2,
          f_r = 0.6) #CAN VARY THIS - DEPENDING ON FACTOR EXPLORED
...
out1$Beta_vv <- beta1(times, 71,(6*7), as.numeric(parms[5])) * 1/3
out1$Beta_vh_hv <- beta1(times, 71, (6*7), as.numeric(parms[5])) * 1/3
out1$Beta_hh_rh_hr <- beta2(times, 71, (6*7), as.numeric(parms[6])) * 1/3
out1$Beta_rr <- beta3(times, 71,(6*7), as.numeric(parms[6])) * 1/3
out1$Beta_rv_vr <- beta4(times, 71 , (6*7), as.numeric(parms[5])) * 1/3

colnames(out1) <- c("Time", "Suscv", "Susch", "Suscr", "Infected_Iv", "Infected_Ih", "Infected_Ir", 
                    "Recovv", "Recovh", "Recovr", "Beta_vv", "Beta_vh_hv", "Beta_hh_rh_hr", "Beta_rr", "Beta_rv_vr")
...
pbeta1 <- ggplot(data = statsbeta1, aes(x = (Time), y = value, col = variable)) + geom_line(size = 1.02, stat = "identity") +
  labs(x ="Time (Days)", y = "Beta_vv") + scale_y_continuous(limits = c(0,0.3),  expand = c(0,0)) +
...
pbeta2 <- ggplot(data = statsbeta2, aes(x = (Time), y = value, col = variable)) + geom_line(size = 1.02, stat = "identity") +
  labs(x ="Time (Days)", y = "Beta1_vh/hv") + scale_y_continuous(limits = c(0,0.3),  expand = c(0,0)) +
  theme(legend.position = "none", legend.title = element_text(size=14), legend.text=element_text(size=14),  axis.text=element_text(size=14),
        axis.title.y=element_text(size=14),axis.title.x= element_blank(), 
        legend.spacing.x = unit(0.3, 'cm'), plot.margin=unit(c(0.7,0.7,0.8,0.8),"cm")) + scale_x_continuous(expand = c(0, 0)) 

pbeta3 <- ggplot(data = statsbeta3, aes(x = (Time), y = value, col = variable)) + geom_line(size = 1.02, stat = "identity") +
  labs(x ="Time (Days)", y = "Beta_hh/hr/rh") + scale_y_continuous(limits = c(0,0.3),  expand = c(0,0)) +
  theme(legend.position = "none", legend.title = element_text(size=14), legend.text=element_text(size=14),  axis.text=element_text(size=14),
        axis.title.y=element_text(size=14),axis.title.x= element_blank(), 
        legend.spacing.x = unit(0.3, 'cm'), plot.margin=unit(c(0.7,0.7,0.8,0.8),"cm")) + scale_x_continuous(expand = c(0, 0)) 

pbeta4 <- ggplot(data = statsbeta4, aes(x = (Time), y = value, col = variable)) + geom_line(size = 1.02, stat = "identity") +
  labs(x ="Time (Days)", y = "Beta_rr") + scale_y_continuous(limits = c(0,0.3),  expand = c(0,0)) +
  theme(legend.position = "none", legend.title = element_text(size=14), legend.text=element_text(size=14),  axis.text=element_text(size=14),
        axis.title.y=element_text(size=14),axis.title.x= element_blank(), 
        legend.spacing.x = unit(0.3, 'cm'), plot.margin=unit(c(0.7,0.7,0.8,0.8),"cm")) + scale_x_continuous(expand = c(0, 0)) 

pbeta5 <- ggplot(data = statsbeta5, aes(x = (Time), y = value, col = variable)) + geom_line(size = 1.02, stat = "identity") +
  labs(x ="Time (Days)", y = "Beta_rv/vr") + scale_y_continuous(limits = c(0,0.3),  expand = c(0,0)) +
  theme(legend.position = "none", legend.title = element_text(size=14), legend.text=element_text(size=14),  axis.text=element_text(size=14),
        axis.title.y=element_text(size=14),axis.title.x= element_text(size=14), 
        legend.spacing.x = unit(0.3, 'cm'), plot.margin=unit(c(0.7,0.7,0.8,0.8),"cm")) + scale_x_continuous(expand = c(0, 0)) 
