...
for(j in 1:length(seq(1,5))) {
  datalist[[j]] <- local({
    ...
    datatext <- data.frame(x = c(200, 200), y = c(0.1875, 0.165), label = c( paste0("italic(I)[italic(max)]", " ==", peak), 
    ...
    if(parms[["scen"]]  == 1) {
      ...
      p2 <- p2 + labs(x ="Time (Days)", y = bquote(beta* "(t)"), col = "")
    ...
    combplot <- ggarrange(p1,p2, nrow = 2, ncol = 1, common.legend = TRUE, legend = "none", align = "v",heights = c(1, 0.45, 0.5))
...
ggsave(combplotmulti, filename = "5_scenarios_multi_JL.png", dpi = 300, type = "cairo", width = 16, height =6, units = "in")
...
multicombplot <- ggarrange(p1, NULL, p2, ncol = 3, nrow = 1, widths = c(1,0.03,0.9), align = "h")


#combplot <- ggarrange(scensens[[1]][[1]],scensens[[2]][[1]],scensens[[3]][[1]],scensens[[4]][[1]],scensens[[5]][[1]],
#                      nrow = 5, ncol = 1)

#ggsave(multicombplot, filename = "Heat_5_scenarios_sensitivity_JL.png", dpi = 300, type = "cairo", width = 10, height = 16, units = "in")
