...
times <- seq(0,147,by = 1)
duration <- 3 # Duration of the intervention
effic <- c((1.5-(0.9*1))/1.5,
           (1.5-(0.9*0.8))/1.5,
           (1.5-(0.9*0.6))/1.5,
           (1.5-(0.9*0.4))/1.5,
           (1.5-(0.9*0.2))/1.5,
...
EfficPerc <- c("100%","80%","60%","40%","20%","0%")

for(i in 1:length(effic)) {
  temp <- data.frame(matrix(nrow = length(seq(0,147)), ncol = 6))
  ...
  temp[,1] <- EfficPerc[i]
  temp[,2] <- c(seq(-100,0), seq(1,147-100))
  temp[,3] <- out1$I 
  temp[,4] <- out1$R 
  temp[,5] <- out1$C 
...
p11 <- ggplot(data = stats1[which(stats1$variable == "Infec"),], aes(x = (Time), y = value, col = Lockdown_Efficacy)) + geom_line(size = 1.02, stat = "identity") +
  ...
  scale_color_manual(values=c("black", "#1F78B4", "#B2DF8A", "#33A02C", "#FB9A99", "darkred"))
