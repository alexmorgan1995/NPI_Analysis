...
for(i in 1:length(trigday)) {
  parms = c(mu = 1/(GenTime(6,2)), int_timestart = trigday[i], int_timeend = trigday[i]+(12*7), beta_base = 2*(1/(GenTime(6,2)))*0.25,
...
out2 <- data.frame(ode(y = init, func = SIR1, times = times, parms = parms))
out2$beta <- betastatdecrease(times, as.numeric(parms[2]), as.numeric(parms[3]))
