...
data_ABC <- ABC_algorithm(N = 1000, 
                          epsilon = c((63*0.025), (0.02566227*0.025)),
...
hist(runif(2500, min = 0, max = 1),xlab = "SIVR Model Parameter Values")

plot(density(runif(2500, min = 0, max = 1)), xlim = c(0, 1), lwd = 1.2, xlab = "Beta") 

closefit <- read.csv("COVID_fit_close.csv")
...
#Beta_AA
betaAA <- data.frame("value" = closefit[,2])
betaAA_MAP <- data.frame("MAP" = map_estimate(closefit[,2], precision = 1000))
...
mu <- data.frame("value" = closefit[,3])
mu_MAP <- data.frame("MAP" = map_estimate(closefit[,3], precision = 1000))

ggplot(mu, aes(x=value)) + geom_density(alpha=.3, fill = "red")  + 
  geom_vline(data = mu_MAP, aes(xintercept = MAP), size=1.05, linetype = "dashed") +
  scale_x_continuous(expand = c(0, 0), limits = c(0,1)) + scale_y_continuous(expand = c(0, 0), limits = c(0,6))+
  geom_text(data = mu_MAP, aes(x = MAP + 0.07, label= signif(MAP, digits = 3), y= 5.5), show.legend=FALSE, size = 5) +
  labs(x ="Mu (1/Generation Time)", y = "Density") + theme(plot.margin=unit(c(0.5,0.5,0.5,0.5),"cm"))

#Proportion of non-Visible Infections - v
vparm <- data.frame("value" = closefit[,4])
vparm_MAP <- data.frame("MAP" = map_estimate(closefit[,4], precision = 1000))
...
parms1 = c(mu = 0.376, beta = 0.487, v = 0.094)
