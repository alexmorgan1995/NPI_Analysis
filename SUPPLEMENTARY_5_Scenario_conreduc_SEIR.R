...
for(j in 1:length(seq(1,5))) { #For each of the 5 scenarios conduct a model run
  datalist[[j]] <- local({
    ...
    #Combine the trajectory plot, beta(t) and re(t) plots together
    combplot <- ggarrange(p1,p2,p3, nrow = 3, ncol = 1, common.legend = TRUE, legend = "none", align = "v",heights = c(1, 0.4, 0.5))
    dump <- list(combplot, plotdata, max(out$C)) #Retaining the plots and dataframes needed outside of the local() for-loop
    return(dump)
  })
}
...
#Identify the peak prevalence and the attack rate for each scenario. 
for(i in 1:5) {
...
#Save the final trajectory plot int he working directory
...
for(z in 1:5) { #Run for each of the 5 scenarios and store each trajectory plot separately
  outcomelistcmin[[z]] <- local({
    ...
    for(i in 1:3) { #Run for each parameter explored 
      sensitivity <- sens #Define what parameter you are exploring 
